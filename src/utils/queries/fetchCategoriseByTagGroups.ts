...
WITH ActivityData AS (
  SELECT 
    ...
    l.latitude, 
    l.longitude,
    JSONB_OBJECT_AGG(i.image_type, i.image_url)
...
SELECT 
  ...
  JSONB_AGG(DISTINCT ad)
    FILTER (WHERE ad.activity_id IS NOT NULL) AS activities
FROM tag_group tg
JOIN tag t ON t.tag_group_id = tg.tag_group_id
LEFT JOIN activity_tag at ON at.tag_id = t.tag_id
LEFT JOIN activity_location al ON al.activity_location_id = at.activity_location_id
LEFT JOIN ActivityData ad ON ad.activity_id = al.activity_id
